...
const T = {
  ar: {
    home:'الرئيسية', reference:'مرجع سريع', theory:'نظري', trial:'محاكاة الاختبار',
    choosePath:'اختر مسار المذاكرة', choosePathSub:'كل ما تحتاجه لمادة أنماط التصميم — نظرياً وعملياً',
    practicalPath:'عملي', practicalSub:'١٠ أنماط بتطبيق عملي + كتابة كود + محاكاة اختبار اللاب', practicalGo:'ابدأ التطبيق',
    theoryPath:'نظري', theorySub:'كل الـ ٢٣ نمط: شرح مفصّل + سيناريوهات + روائح الكود + مراجعة', theoryGo:'ابدأ المراجعة',
    ...
    introductory:'تمهيدي', intermediate:'متوسط', advanced:'متقدم', expert:'خبير',
    studyCarefully:'تم عرض الحل — ادرسه بعناية',
    resetConfirm:'إعادة تعيين إلى الكود الأساسي؟', showSolConfirm:'عرض الحل؟',
    readOnly:'للقراءة فقط', tab4:'Tab = 4 مسافات',
  ...
  en: {
    home:'Home', reference:'Reference', theory:'Theory', trial:'Mock Exam',
    choosePath:'Choose your study path', choosePathSub:'Everything you need for Design Patterns — in theory and practice',
    practicalPath:'Practical', practicalSub:'10 patterns with hands-on coding + mock lab exam', practicalGo:'Start practicing',
    theoryPath:'Theory', theorySub:'All 23 patterns: detailed explanation + scenarios + code smells + review', theoryGo:'Start reviewing',
...
// Removes Unicode bidi/override controls that enable RLO spoofing
// U+202A–202E (embeddings/overrides), U+2066–2069 (isolates), U+200E/200F/061C (marks)
function stripBidiControls(s){return String(s??'').replace(/[‪-‮⁦-⁩‎‏؜]/g,'');}
// Wraps dynamic inline text as an isolated BiDi run (safe ordering + escaped)
...
const App={
  ...
  buildNav(){
    ...
    if(navTheory)navTheory.textContent=t('theory');
    ...
    document.querySelectorAll('.lang-btn').forEach(b=>{
      const isActive=b.dataset.l===LANG;
  ...
  route(){
    ...
    else if(p[0]==='theory'){ window.Theory && Theory.route(app,p); }
    else if(p[0]==='trial'){
...
function renderHome(app){
  ...
  <div class="hero-cosmos fade-in">
    <div class="hero-content">
      ...
      <h1 class="hero-title-big"><span class="accent-grad">Design Patterns</span></h1>
      <div class="hero-cta">
  ...
  <div class="path-chooser fade-in">
    <div class="path-chooser-head">
      <h2 class="path-chooser-title ${textClass()}">${t('choosePath')}</h2>
      <p class="path-chooser-sub ${textClass()}">${t('choosePathSub')}</p>
    </div>
    <div class="path-cards">
      <button class="path-card path-card-practical" data-goto="reference">
        <div class="path-card-icon">⚙</div>
        <div class="path-card-name">${t('practicalPath')}</div>
        <div class="path-card-sub ${textClass()}">${t('practicalSub')}</div>
        <div class="path-card-go">${t('practicalGo')} →</div>
      </button>
      <button class="path-card path-card-theory" data-goto="theory">
        <div class="path-card-icon">🧠</div>
        <div class="path-card-name">${t('theoryPath')}</div>
        <div class="path-card-sub ${textClass()}">${t('theorySub')}</div>
        <div class="path-card-go">${t('theoryGo')} →</div>
      </button>
    </div>
  </div>
...
function renderPattern(app,id,tab,sub){
  ...
  setTimeout(()=>{const activeTab=app.querySelector('.tab.active');if(activeTab)activeTab.scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'});},80);
  ...
  if(uml&&uml.classes){
    ...
    umlHTML=`<div class="uml-wrapper">
      ...
      ${relBadges?`<div style="padding:0.75rem;background:var(--surface2);border-radius:var(--r-sm);border:1px solid var(--border);">
        <div style="font-family:var(--font-code);font-size:0.62rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--text-muted);margin-bottom:0.5rem;">${LANG==='ar'?'العلاقات':'Relations'}</div>
        ${relBadges}
      </div>`:''}
    </div>`;
  }
...
function renderCode(id,pattern){
  ...
  <div class="learn-layout">
    <aside class="learn-sidebar">
      <div class="learn-sidebar-label">${LANG==='ar'?'الكلاسات':'Classes'}</div>
  ...
  const methods=cls.methods.map((m,i)=>`
    <div class="method-block ${i===0?'open':''}" data-mi="${i}">
      ...
      <div class="method-body">
        ...
        <div class="method-snippet"><pre>${hi(m.code)}</pre></div>
      </div>
    </div>`).join('');
  detail.innerHTML=`
    ...
    return`<div class="pitfall-card">
      ...
      <div class="pitfall-body">
        <div class="pitfall-compare">
          <div>
            <div class="pitfall-label" style="color:var(--err);direction:${textDir()};text-align:${LANG==='ar'?'end':'start'};">${t('wrong')}</div>
            <pre class="pitfall-bad">${esc(p.bad)}</pre>
          </div>
          <div>
            <div class="pitfall-label" style="color:var(--ok);direction:${textDir()};text-align:${LANG==='ar'?'end':'start'};">${t('correct')}</div>
...
function renderPractice(pid,pattern,levelId){
  ...
  const dbtns=DIFF.map(d=>{
    ...
    return`<div class="diff-btn ${d.d===levelId?'active':''}" data-d="${d.d}" data-level="${d.d}">
      ...
      <div><div class="diff-label">${LANG==='ar'?d.ar:d.en}</div><div class="diff-label-ar">${LANG==='ar'?d.en:''} ${ldone}/${ltotal}</div></div>
    </div>`;
  }).join('');
...
function renderLevelEditor(pid,pattern,levelId){
  ...
  pb.innerHTML=`
  <div class="task-card"><div class="task-card-title ${textClass()}">${LANG==='ar'?'المستوى':'Level'} ${levelId} · ${DIFF[levelId-1][LANG==='ar'?'ar':'en']}</div><div class="${textClass()}" style="direction:${textDir()}">${LANG==='en'&&level.taskEn?level.taskEn:level.task}</div></div>
  <div class="practice-wrap">
    <div class="file-panel"><div class="file-panel-title">${LANG==='ar'?'الملفات':'Files'}</div>${files}</div>
...
function renderNotes(pid,pattern){
  ...
  tc.innerHTML=`<div style="max-width:700px;">
    <p class="${tc2}" style="color:var(--text-secondary);font-size:0.87rem;margin-bottom:0.75rem;direction:${dir}">${t('myNotes')} <strong>${bdi(pattern.name)}</strong> — ${t('savedAuto')}</p>
  ...
  document.getElementById('notes-ta').addEventListener('input',e=>{
    const clean=stripBidiControls(e.target.value);
    if(clean!==e.target.value){const pos=e.target.selectionStart;e.target.value=clean;e.target.setSelectionRange(pos-1,pos-1);}
    clearTimeout(timer);
    timer=setTimeout(()=>{Store.set('notes_'+pid,clean);const ind=document.getElementById('notes-ind');if(ind){ind.classList.add('show');setTimeout(()=>ind.classList.remove('show'),1400);}if(clean.length>30)window.ACH&&ACH.trackNoteWritten(pid);},600);
  });
}
...
const Trial={
  ...
  _showSol(){
    ...
    modal.innerHTML=`<div class="modal">
      ...
      <div class="modal-title ar">${t('solution')} — ${q.title}</div>
...
const JavaRunner={
  ...
  _topbar(outputId){
    return`<div class="run-topbar"><div class="run-topbar-left"><div class="run-dots"><div class="run-dot"></div><div class="run-dot"></div><div class="run-dot"></div></div><span class="run-label">${LANG==='ar'?'المخرجات':'Output'}</span></div><button class="run-close-btn" onclick="document.getElementById('${outputId}').className='run-output'" aria-label="إغلاق">×</button></div>`;
  },
