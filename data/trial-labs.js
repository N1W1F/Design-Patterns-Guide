...
// غير كذا فوّض للـ realDatabase`,
  ...
  // ==================== LAB 9: Prototype - Game Enemies ====================
  {
    ...
    scenario: `نظام إدارة وثائق فيه قوالب جاهزة (Templates) من أنواع مختلفة: Report و Invoice. كل قالب إنشاؤه غالي (يحتاج تحميل header, footer, styles, و metadata من قاعدة البيانات). المستخدم يطلب نفس القالب مراراً، فبدل ما نعيد بناءه من الصفر في كل مرة، نحتاج نسخ القوالب الموجودة.`,
    task: `<strong>المطلوب:</strong> طبّق <strong>Prototype pattern</strong>:
    <br>• <code>Document</code> abstract class فيه title و author (Strings) + <code>clone()</code> abstract
    <br>• <code>Report</code> + <code>Invoice</code> ينفذون clone عبر <strong>copy constructor</strong>
    <br>• <code>DocumentRegistry</code> فيه HashMap يخزن prototypes ويُرجع clones`,
    starterFiles: [
      {
        name: 'Document.java',
        status: 'todo',
        starter: `// TODO: abstract Document فيه title (String) و author (String) + clone() abstract`,
    ...
    public abstract Document clone();
        ...
        checks: [
          { type: 'contains', value: 'abstract class Document', msg: 'abstract class' },
          { type: 'regex', value: 'private\\s+String\\s+title', msg: 'field title' },
          { type: 'regex', value: 'private\\s+String\\s+author', msg: 'field author' },
          { type: 'regex', value: 'public\\s+abstract\\s+Document\\s+clone', msg: 'abstract clone()' }
        ]
      },
      {
        name: 'Report.java',
        status: 'todo',
        starter: `// TODO: Report extends Document فيه summary (String)`,
    ...
    public Document clone() {
      ...
      },
      {
        name: 'Invoice.java',
        status: 'todo',
        starter: `// TODO: Invoice extends Document فيه amount (double)`,
        solution: `public class Invoice extends Document {
    ...
    public Document clone() {
        ...
        checks: [
          { type: 'contains', value: 'extends Document', msg: 'extends Document' },
          { type: 'regex', value: 'Invoice\\s*\\(\\s*Invoice\\s+\\w+\\s*\\)', msg: 'copy constructor Invoice(Invoice)' },
          { type: 'contains', value: 'new Invoice(this)', msg: 'clone() تُرجع new Invoice(this)' }
        ]
      },
      {
        name: 'DocumentRegistry.java',
        status: 'todo',
        starter: `import java.util.HashMap;
import java.util.Map;

// TODO: DocumentRegistry يخزن prototypes + get(key) تُرجع clone`,
...
public class DocumentRegistry {
    ...
    public DocumentRegistry() {
        Report report = new Report("Monthly Report", "Admin", "Summary text");
        Invoice invoice = new Invoice("Invoice Template", "Billing", 0.0);
        registry.put("report", report);
        registry.put("invoice", invoice);
    }
    ...
    public Document get(String key) {
        ...
        checks: [
          ...
          { type: 'contains', value: 'registry.put', msg: 'put prototypes في الـ registry' },
          { type: 'contains', value: '.clone()', msg: 'get() تُرجع clone وليس الأصل' }
        ]
      }
    ],
    solutionSteps: [
      {
        title: '1. تحديد الـ Pattern',
        content: 'مؤشرات Prototype: نسخ كائنات موجودة + بناء غالي (DB calls) + cache من قوالب جاهزة = Prototype بدون تردد.'
      },
      {
        title: '2. Abstract Base',
        content: 'Document abstract فيه common fields (title, author) + <code>abstract Document clone()</code>. هذا العقد المشترك لكل أنواع المستندات.'
      },
      {
        title: '3. Copy Constructor',
        content: 'في كل subclass (Report, Invoice)، أضف <strong>copy constructor</strong> يأخذ نفس النوع: <code>Report(Report target)</code>. يستدعي super بقيم target ثم ينسخ الـ fields الخاصة.'
      },
      {
        title: '4. clone() Implementation',
        content: 'في كل subclass: <code>public Document clone() { return new Report(this); }</code>. هذا يستخدم الـ copy constructor.'
      },
      {
        title: '5. Registry',
        content: 'DocumentRegistry فيه HashMap. الـ constructor يبني prototypes جاهزة ويضعها في الـ map. <code>get(key)</code> تُرجع <strong>clone</strong> وليس الأصل — هذا الفرق الجوهري.'
      }
    ]
  },
