...
reader.close();  // كل طبقة تنظف نفسها بالعكس`
  ...
  flyweight: {
    ...
    insight: {
      ...
      en: 'Split the state into two kinds: intrinsic (shared, immutable: texture, mesh) and extrinsic (per-instance: position, height). Share intrinsic via a cache, store only extrinsic in each instance.'
    },
    mechanics: [
...
// لو 1M JSON object فيه "name", "email", "age" → 3 strings مشتركة لا 3M`
  ...
  proxy: {
    ...
    insight: {
      ...
      en: 'A surrogate (Proxy) implements the same interface as the real object, sits between it and the client, decides when/if/how to forward calls. Transparent in terms of interface, but adds control behavior (auth, caching, etc.).'
    },
    mechanics: [
...
// Lambda طريقة مختصرة لتطبيق Strategy`
    },
    relations: [
